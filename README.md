...
Архитектура доступа к данным целиком построена на RLS-политиках Postgres: например, увидеть чужой пост, реакцию или комментарий можно только если между пользователями есть подтверждённое «знакомство» — это проверяется на уровне базы, а не в коде приложения.
