...
Счётчик комментариев на странице ленты считается батч-запросом (`comments` с `post_id in (...)`), не по одному запросу на пост. Счётчики реакций с 0.4.0 приходят одним вызовом `security definer`-функции `reaction_summary()` (числа + собственная реакция, без чужих `user_id`) — раньше это тоже был батч-запрос по `reactions`, но он отдавал наружу, кто что поставил.
...
**CI/CD для сборок (по желанию, не обязательно для портфолио):** можно оставить ручную сборку через Android Studio/Xcode для 0.1 — автоматизация подписи и публикации через Fastlane/Codemagic имеет смысл добавить в 1.0, когда сборки станут частыми. Для одного релиза ради портфолио ручной процесс проще для новичка и не потребует отладки CI-специфичных проблем с сертификатами.
